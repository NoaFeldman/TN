...
class Spectral {
 public:
    ...
    unsigned linfit=1, const char *isbuf="") const;
...
void Spectral::getOmega(wbvector<double> &om) const {
   unsigned linfit=0;
   getOmega(om,linfit);
}

void Spectral::getOmega(
   wbvector<double> &om,
   unsigned &linfit
) const {
...
void Spectral::pairRawData() {
   ...
   "ERR Ap has even number of rows !?? (%d)", Ap.dim1);
...
void Spectral::dispSumRule() {
   ...
   wblog(FL,
       "Sum %sp_raw      = %s\n"
       " �  %sn_raw      = %s",
   istr.data, sp.toStrf("%8.6f").data, istr.data, sn.toStrf("%8.6f").data);
   ...
   if (i==sp.len) {
       sp-=m; wblog(FL,
      "\e[34m �  total-%g     = %s\e[0m", m, sp.toStrf("%8.2g").data);
   } else wblog(FL,
      "\e[34m �  total       = %s\e[0m", sp.toStrf("%8.6f").data);
}
...
void Spectral::dispSumRule(
   ...
   wblog(FL,"Sum AA_smooth   = %s",s.toStrf("%8.6f").data);

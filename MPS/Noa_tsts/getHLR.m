function HLR = getHLR(H, psi, l, dir, HLR, full)
    % TODO
    if (strcmp(dir, '>>'))
        if (l == 1)
    ...
    if (strcmp(dir, '<<'))
        if (l == length(psi))
            HLR.identities = getIdentity(psi(l), 3, [2, 1]);
            HLR.opSum = QSpace;
            HLR.openOp = QSpace;
        end
        % TODO document with drawings
        thisSiteHSingle = contract(contract(H.single(l), 1, psi(l), 2), 1, psi(l), '2*');
        thisSiteHSingle = contract(HLR.identities, '12', thisSiteHSingle, '24');
        thisSiteIdentity = contract(contract(H.identity(l), 1, psi(l), 2), 1, psi(l), '2*');
        if (l ~= length(psi))
        ...
        thisSiteHR2L = contract(contract(H.r2l(l), 1, psi(l), 2), 1, psi(l), '2*');
        HLR.openOp = contract(HLR.identities , '12', thisSiteHR2L, '35');
        
        HLR.opSum = HLR.opSum + thisSiteHSingle;
        HLR.identities = contract(HLR.identities, '12', thisSiteIdentity, '24'); 
    end
